...
private final class AESCipher {
    ...
    func encrypt(bytes: [UInt8]) throws -> [UInt8] {
        ...
        return try blockMode.encrypt(blocks: blocks, iv: iv, cipherOperation: encrypt)
    }
    ...
    func decrypt(bytes: [UInt8]) throws -> [UInt8] {
        ...
        return try PKCS7().remove(bytes: blockMode.decrypt(blocks: blocks, iv: iv, cipherOperation: decrypt), blockSize: AESCipher.blockSize)
    }
    ...
    fileprivate func expandKeyInv(_ key: Key, variant: Variant) -> [[UInt32]] {
        let rounds = variant.Nr
    ...
    fileprivate func expandKey(_ key: Key, variant: Variant) -> [[UInt32]] {
...
private struct PKCS7 {
    ...
    func remove(bytes: [UInt8], blockSize _: Int?) -> [UInt8] {
...
private extension Array {
    ...
    func chunks(size chunksize: Int) -> [[Element]] {
        ...
        let reminder = suffix(count % chunksize)
        if !reminder.isEmpty {
